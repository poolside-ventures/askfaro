...
/// Infer the operation from the fields present, when the caller omitted it.
///
/// Not leniency for its own sake, and not a new idea: `units` already defaults
/// to `convert`, `calc` to `evaluate` and `phone` to `parse`. This namespace
/// simply never got the same treatment, and the omission is the single most
/// common way a model gets a correct call rejected. Measured on a real
/// on-device run: the model sent `datetime`, `from_timezone` and `to_timezone`,
/// every field name correct and taken from the published contract, and lost the
/// turn to a missing `operation`.
///
/// Only unambiguous shapes are inferred. A pair of zones plus a datetime can
/// only be a conversion; a lone `timezone` can only be a current-time lookup.
/// Anything else still errors with the list of operations, because guessing
/// between two plausible readings is how a tool returns a confident wrong
/// answer instead of a usable complaint.
fn infer_operation(params: &serde_json::Value) -> Option<&'static str> {
    let has = |k: &str| params.get(k).is_some();
    if has("from_timezone") && has("to_timezone") {
        return Some("convert");
    }
    if has("timezone") && !has("from_timezone") && !has("to_timezone") {
...
pub(crate) fn run(params: serde_json::Value) -> ToolOutput {
    let explicit = params.get("operation").and_then(|v| v.as_str()).unwrap_or("");
    let op = if explicit.is_empty() {
        infer_operation(&params).unwrap_or("")
    } else {
        explicit
    };
    match op {
